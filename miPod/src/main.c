...
void share_song(char *song_name, char *username) {
  ...
  while (written < length) {
    wrote = write(fd, (char *)&c->song.wav_md + written, length - written);
    if (wrote == -1) {
...
int play_song(char *song_name) {
  ...
  if (!load_file(song_name, (void *)&c->song.wav_md)) {
    mp_printf("Failed to load song!\r\n");
    return 0;
  }
...
void digital_out(char *song_name) {
  ...
  int written = 0, wrote, length = c->song.wav_md.file_size + 8;
