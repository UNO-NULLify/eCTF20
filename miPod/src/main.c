...
void print_help() {
  ...
  mp_printf("  login <username> <pin>: log on to a miPod account (must be "
            "logged out)\r\n");
  mp_printf("  logout: log off of a miPod account (must be logged in)\r\n");
  mp_printf("  query <song.drm>: display information about the song\r\n");
  mp_printf("  share <song.drm> <username>: share the song with the specified "
            "user\r\n");
...
void query_song(char *song_name) {
  ...
  if (!load_file(song_name, (void *)&c->drm)) {
...
void share_song(char *song_name, char *username) {
  ...
  while (written < length) {
    wrote = write(fd, (char *)&c->drm.s_md + written, length - written);
    if (wrote == -1) {
...
int play_song(char *song_name) {
  ...
  if (!load_file(song_name, (void *)&c->drm.s_md)) {
    mp_printf("Failed to load song!\r\n");
    return 0;
  }
...
void digital_out(char *song_name) {
  ...
  int written = 0, wrote, length = c->drm.s_md.file_size + 8;
...
int main(int argc, char **argv) {
  ...
  c = mmap(NULL, sizeof(cmd_channel), PROT_READ | PROT_WRITE, MAP_SHARED, mem,
           0);
  if (c == MAP_FAILED) {
