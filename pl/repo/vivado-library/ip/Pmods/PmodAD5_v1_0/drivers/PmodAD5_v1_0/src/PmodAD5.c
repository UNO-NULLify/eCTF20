...
XSpi_Config AD5Config = {0, 0, 1, 0, 1, 8, 0, 0, 0, 0, 0};
/* ------------------------------------------------------------ */
/***	void AD1_begin(PmodAD5* InstancePtr, u32 BaseAddress)
**
**	Parameters:
**		InstancePtr: A PmodAD5 object to start
**		BaseAddress: The base address of the PmodAD5 AXI_LITE_SAMPLE
*interface
**
**	Description:
**		Initialize the PmodAD5 device - note that the AD5 IP is
*free-running, and this function just prepares the driver for use. *
*Also calls AD5_WriteConfig which configures the AD5 for a gain of 1 and set to
*differential.
*/
...
int AD5_SPIInit(XSpi *SpiInstancePtr) {
  ...
  Status =
      XSpi_CfgInitialize(SpiInstancePtr, &AD5Config, AD5Config.BaseAddress);
 ...
 **
 **     Description:
 **             Reads data in through SPI. It sends the first byte to the
 *communication register which sets communication to read for
 **             the next 3 bytes.
 **             Data is stored into data1, data2 and data3.
 */
void AD5_readData(PmodAD5 *InstancePtr) {
  u8 bytes[4];
  bytes[0] = 0x58; // first byte is sent to the communication register which
                   // sets communication to
 ...
 **
 **     Description:
 **             writes data to the configuration register through SPI. It sends
 *the first byte to the communication
...
void AD5_WriteConfig(PmodAD5 *InstancePtr) {
  u8 bytes[4];
  bytes[0] = 0x10; // first byte is sent to the communication register which
                   // sets communication to writing the next 3 bytes to the
 ...
 **
 **     Description:
 **             Combines the 3 bytes into a unsigned long integer and then
 *converts the digital data into a float voltage value
 */
...
float AD5_DataToVoltage(PmodAD5 *InstancePtr) {
  ...
  voltage =
      (((float)rawdata / (float)8388608) - (float)1) * (mVref / (float)PGAGain);
