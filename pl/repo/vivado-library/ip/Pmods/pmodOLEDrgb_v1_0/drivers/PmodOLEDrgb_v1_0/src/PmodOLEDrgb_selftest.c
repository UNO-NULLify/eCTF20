 ...
 *
 * @param   baseaddr_p is the base address of the PMODOLEDRGBinstance to be
 * worked on.
 *
 * @return
 *
...
XStatus PMODOLEDRGB_Reg_SelfTest(void *baseaddr_p) {
  ...
  for (write_loop_index = 0; write_loop_index < 4; write_loop_index++)
    PMODOLEDRGB_mWriteReg(baseaddr, write_loop_index * 4,
                          (write_loop_index + 1) * READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0; read_loop_index < 4; read_loop_index++)
    if (PMODOLEDRGB_mReadReg(baseaddr, read_loop_index * 4) !=
