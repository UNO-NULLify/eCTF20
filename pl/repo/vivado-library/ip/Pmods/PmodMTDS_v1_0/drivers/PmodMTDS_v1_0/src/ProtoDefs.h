/********************************************************************************/
/*																				*/
/*	ProtoDef.h	--	Display Shield Communications Protocol
 * Definitions			*/
/*																				*/
/********************************************************************************/
 ...
 */
/*																				*/
/*	This header file describes the communications protocol used by the
 * Digilent	*/
 ...
 */
/*																				*/
/*	Communications between the host processor and the display shield
 * processor	*/
/*	is via an SPI port. The host sends command and data packets to the
 * display	*/
/*	shield processor and the display shield processor sends status and data
 */
/*	packets back to the host via the SPI interface. There is also an
 * interrupt	*/
/*	pin that the display shield uses to signal to the host that there is
 * some	*/
...
** rather than packets. These are used to drive the communications channel
*rather
...
#define chnCmdStart                                                            \
  (clsProtoChan + 0x02) // tell firmware applccation to start running
...
#define chnStaDone                                                             \
  (clsProtoChan + 0x22) // shield has finished processing command
#define chnStaReady (clsProtoChan + 0x23) // ready to receive data out packet
#define chnStaAbort                                                            \
  (clsProtoChan + 0x24) // shield requests to abort current command
#define chnStaSync                                                             \
  (clsProtoChan + 0x25) // shield response to channel sync request
#define chnStaStartup                                                          \
  (clsProtoChan + 0x26) // shield hasn't compeleted startup init
...
#define chnStaTimeout                                                          \
  (clsProtoChan + 0x3E) // timeout waiting for shield to respond
#define chnStaDataSize                                                         \
  (clsProtoChan + 0x3F) // cb field in packet header out of range
...
#define cmdUtilSetDspOrientation                                               \
  0x06 // set display to portrait/landscape orientation
...
#define cmdUtilBeginUpdate                                                     \
  0x1F // pass parameters and set up for firmware update
...
#define cmdGdiDestroyBitmap                                                    \
  0x0A                         // delete off-screen bitmap when no longer needed
...
#define cmdGdiGetNearestColor                                                  \
  0x27                        // get nearest realizable color to specified color
...
/* Generic error codes start with 0x80. Subsystem specific error codes start
 * with 0x01.
 */
...
#define staUtilInvalidMemoryHandle                                             \
  0x04 // bad memory handle passed to a memory function
...
#define staFsInvalidObject                                                     \
  0x09                         // file/directory object invalid (internal error)
#define staFsWriteProtect 0x0A // physical drive is write protected
#define staFsInvalidDrive                                                      \
  0x0B // logical drive number is invalid (internal error)
