...
XSpi MTDS_Spi;
XTmrCtr Timer;
XTmrCtr_Config Timer_Config = {0, 0,
...
XSpi_Config MTDSSPIConfig = {
    0, XPAR_PMODMTDS_0_AXI_LITE_SPI_BASEADDR, 1, 0, 1, 8, 0, 0, 0, 0, 0};
...
**
**	Description:
**		This function performs any system initialization required for
*the *		platform. This can include initializing timers, settting up pin
**		direction, and so on. The pinSel parameter is specifically used
*in *		the Arduino/MPIDE environment and may or may not be used on
*other *		platforms.
*/

void MtdsHalInit(int pinSel) {

  XTmrCtr_CfgInitialize(&Timer, &Timer_Config,
                        XPAR_PMODMTDS_0_AXI_LITE_TIMER_BASEADDR);
  ...
  Xil_Out32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 4,
            0b11); // Set IntA and IntB to inputs
  Xil_Out32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 8, 0b111);
  Xil_Out32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 12,
            0b000); // Set chip selects(ch2) to outputs
}
...
**
**		When building for the Multi-Touch Display Shield, the pin
*numbers for
**		the status pins are defined in the MtdsHal.h header file. When
*building *		for the PmodMtds, we have to compute the pin numbers
*based on the Pmod *		connector the board is plugged into. We can get
*this as a constant offset *		from the pin number of ch SPI CS pin,
*pinMtdsSel. For now, we assume that
**		if we are being built for a board with Pmod connectors, we are
*using the *		PmodMtds.
*/
...
void MtdsHalEnableStatusPin(int idPin) {
  ...
  Xil_Out32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 4,
            0b11); // Set interrupts (ch1) as inputs
}
...
bool MtdsHalGetStatusPin(int idPin) { return false; }
...
**		The Multi-Toucn	Display Shield doesn't support software
*controlled
**		reset and is automatically reset whenever the host board is
*reset. *		The PmodMTDS doesn't get reset automatically and needs
*to be reset *		under software control when the system initializes.
**		The pinSel parameter can be used to determine which pin needs to
**		be toggled to reset the display.
*/

bool MtdsHalResetDisplay(int pinSel) { return false; }
...
**
**	Description:
**		This function initializes the SPI port being used by the
*platform *		for communications between the host and the display
*interface. Depending
...
void MtdsHalInitSpi(uint32_t pspiInit, uint32_t frq) {
  ...
  XSpi_CfgInitialize(&MTDS_Spi, &MTDSSPIConfig,
...
void MtdsHalEnableSlave(bool fEn) {
  ...
  if (fEn) {
    Xil_Out32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 8,
              Xil_In32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 8) & 0b011);
  } else {
    Xil_Out32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 8,
              Xil_In32(XPAR_PMODMTDS_0_AXI_LITE_GPIO_BASEADDR + 8) | 0b100);
  }
}
...
uint8_t MtdsHalPutSpiByte(uint8_t bSnd) {
  ...
  XSpi_SetControlReg(&MTDS_Spi,
                     XSpi_GetControlReg(&MTDS_Spi) &
                         ~XSP_CR_TRANS_INHIBIT_MASK); // Start Transfer
  while ((XSpi_IntrGetStatus(&MTDS_Spi) & XSP_INTR_TX_EMPTY_MASK) == 0)
    ;                                                // Wait until TX is empty
  XSpi_IntrClear(&MTDS_Spi, XSP_INTR_TX_EMPTY_MASK); // Clear TX Empty interrupt
  XSpi_SetControlReg(&MTDS_Spi, XSpi_GetControlReg(&MTDS_Spi) |
                                    XSP_CR_TRANS_INHIBIT_MASK); // Stop Transfer
...
*host *		started running. This is used for determining timeout for failed
*communications *		between the host and the display device and for
*timing how long API functions *		take to execute. *
*This function is not strictly required, and can always just return 0 if the
**		hardware resources aren't available to implement it. If it isn't
*implemented,
**		detection of lost communications between the host and the
*display board won't *		work correctly and the host may lock up if
*communications isn't working reliably.
*/

uint32_t MtdsHalTmsElapsed() { return millis(); }
...
**
**	Description:
**		Return the number of microseconds elapsed since reset. This is
*only used *		to time how long MTDS commands take to execute, so if it
*isn't implemented *		or isn't accurate the only problem will be that
*the API function that
**		returns the time for a command to execute won't work correctly.
*/

uint32_t MtdsHalTusElapsed() { return micros(); }
