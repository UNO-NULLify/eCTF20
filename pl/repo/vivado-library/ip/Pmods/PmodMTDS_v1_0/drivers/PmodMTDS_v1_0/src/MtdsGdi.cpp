...
**
**	Description:
**		Get a handle to the DS object used to talk to the physical
*display. *		There is one distinguished DS that is set up when the
*display shield *		is initialized that uses the bitmap associated
*with the physical *		display as its drawing surface.
*/
...
**
**	Description:
**		This releases the specified DS back to the DS cache, or in the
*case *		of the display DS, marks it as not busy.
*/
...
bool MTDS::ReleaseDs(HDS hds) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiReleaseDs, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
HBMP MTDS::CreateBitmap(int16_t dxco, int16_t dyco, int16_t cbpp) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiCreateBitmap, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
bool MTDS::DestroyBitmap(HBMP hbmp) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiDestroyBitmap, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
**	Parameters:
**		hbmp		- handle to the bitmap to destroy
**		pbmpd		- pointer to bitmap dimensions structure to fill
*in
**
**	Return Values:
**		none
...
bool MTDS::GetBitmapDimensions(HBMP hbmp, BMPD *pbmpd) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetBitmapDim, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
**
**	Description:
**		Load the specified bitmap from the specified file on the SD
*card.
*/
...
HBR MTDS::CreateSolidBrush(uint32_t clr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiCreateSolidBrush, sizeof(prm),
                   (uint8_t *)&prm, 0, 0);
...
HBR MTDS::CreatePatternBrush(int16_t ids, uint32_t clrFg, uint32_t clrBg) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiCreatePatternBrush, sizeof(prm),
                   (uint8_t *)&prm, 0, 0);
...
HBR MTDS::CreateCustomBrush(HBMP hbmp) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiCreateCustomBrush, sizeof(prm),
                   (uint8_t *)&prm, 0, 0);
...
bool MTDS::DestroyBrush(HBR hbr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiDestroyBrush, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
**
**	Description:
**		Convert the specified color value to the nearest realizable
*color on *		the display hardware.
*/
...
uint32_t MTDS::GetNearestColor(uint32_t clr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetNearestColor, sizeof(prm),
                   (uint8_t *)&prm, 0, 0);
...
bool MTDS::SetFgColor(HDS hds, uint32_t clr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetFgColor, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Errors:
**		Returns 0 if an error occurs, which corresponds to the color
*black.
**
**	Description:
**		Return the current foreground color for the specified display
*state object.
*/
...
uint32_t MTDS::GetFgColor(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetFgColor, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		Set the specified color as the current background color for the
...
bool MTDS::SetBgColor(HDS hds, uint32_t clr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetBgColor, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		Return the current background color from the specified display
*state *		object.
*/
...
uint32_t MTDS::GetBgColor(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetBgColor, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		Set the specified color as the current transparency color for
*the specified *		display state object.
*/
...
bool MTDS::SetTransColor(HDS hds, uint32_t clr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetTransColor, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
**
**	Description:
**		Return the current transparency color from the specified display
*state *		object.
*/
...
uint32_t MTDS::GetTransColor(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetTransColor, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
**
**	Description:
**		Set the specified value as the intensity to use for
*DrawBitmap(). This
**		defaults to 100%. Values less than 100 will cause the bitmap to
*be drawn *		with reduced intensity. Values greater than 100 will
*cause the bitmap *		to be drawn with increased intensity, intensity
*basically being the
**		relative brightness of the pixels. This is mostly useful for
*drawing *		bitmaps with reduced intensity to "gray" them out.
*Drawing with an *		intensity greater than 100 will cause many
*bitmaps to saturate to white *		very quickly.
*/
...
bool MTDS::SetIntensity(HDS hds, int ity) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetIntensity, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
int MTDS::GetIntensity(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetIntensity, sizeof(prm), (uint8_t *)&prm,
                   0, 0);
...
**
**	Description:
**		Set the specified specified display state object to use the
*specified *		background drawing mode.
*/
...
bool MTDS::SetBkMode(HDS hds, int bkReq) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetBkMode, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
int MTDS::GetBkMode(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetBkMode, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::SetDrwRop(HDS hds, int drw) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetDrwRop, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
int MTDS::GetDrwRop(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetDrwRop, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		Set the specified brush as the background pattern for the
*specified *		display state object.
*/
...
bool MTDS::SetBrush(HDS hds, HBR hbr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetBrush, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		Returns the handle of the brush object selected as the current
*background *		pattern in the specified display state object.
*/
...
HBR MTDS::GetBrush(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetBrush, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::SetFont(HDS hds, HFNT hfnt) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetFont, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		Returns the handle of the font object selected as the current
*font *		in the specified display state object.
*/
...
HFNT MTDS::GetFont(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetFont, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::SetDrawingSurface(HDS hds, HBMP hbmp) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetDrawingSurface, sizeof(prm),
                   (uint8_t *)&prm, 0, 0);
...
**
**	Description:
**		Returns the handle of the bitmap object selected as the current
*drawing *		surface in the specified display state object.
*/
...
HBMP MTDS::GetDrawingSurface(HDS hds) {
   ...
   */
  prm.valA1 = hds;
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetDrawingSurface, sizeof(prm),
                   (uint8_t *)&prm, 0, 0);
...
bool MTDS::SetPixel(HDS hds, int16_t xco, int16_t yco, uint32_t clr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiSetPixel, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
uint32_t MTDS::GetPixel(HDS hds, int16_t xco, int16_t yco) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetPixel, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**		ppnt	- pointer to PNT structure to receive the current
*position
**
**	Return Values:
**		none
...
bool MTDS::GetCurPos(HDS hds, PNT *ppnt) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiGetCurPos, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		Move the current position in the specified display state object
*to *		the specifiec x,y position.
*/
...
*using
**		the current pen, foreground color, background color and
*background drawing *		mode, leaving the current position unchanged.
*/
...
*using
**		the current pen, foreground color, background color and
*background drawing *		mode, updating the current position to be the
...
**		rgpnt		- array of point structures give the vertices of
*the polyline
**
**	Return Values:
**		none
...
**
**	Description:
**		Draw a series of lines from the current position to the
*specified position,
**		using the current pen, foreground color, background color and
*background drawing *		mode, leaving the current position unchanged.
*/
...
**		using the current pen, foreground color, background color and
*background drawing *		mode, updating the current position to be the
*endpoint of the last line segment *		specified by rgpnt.
*/
...
bool MTDS::Arc(HDS hds, int16_t xcoLeft, int16_t ycoTop, int16_t xcoRight,
               ...
               int16_t xcoRad2, int16_t ycoRad2) {
...
**
**	Description:
**		Draws an ellipse tangent to the sides of the specified bounding
*rectangle. *		The perimeter of the ellipse is drawn using the current
*pen, foreground *		color, background color, and background drawing
*mode. The interior of the
**		ellipse if filled using the current brush and raster-op.
*/

bool MTDS::Ellipse(HDS hds, int16_t xcoLeft, int16_t ycoTop, int16_t xcoRight,
                   int16_t ycoBottom) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiEllipse, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::Rectangle(HDS hds, int16_t xcoLeft, int16_t ycoTop, int16_t xcoRight,
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiRectangle, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::RoundRect(HDS hds, int16_t xcoLeft, int16_t ycoTop, int16_t xcoRight,
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiRoundRect, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::PatBlt(HDS hds, int16_t xco, int16_t yco, int16_t dxco, int16_t dyco,
                  uint16_t rop) {
  PRM1A5B prm;
...
bool MTDS::DrawBitmap(HDS hdsDst, int16_t xcoDst, int16_t ycoDst, int16_t dxco,
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiDrawBitmap, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
**
**	Description:
**		This function uses the specified array of characters and the
*currently
**		selected font in the display state object to determine the width
*and *		height of the rectangle needed to hold the text when it has been
**		rendered.
*/
...
**
**	Description:
**		Render the specified text string using the currently selected
*font.
**		It will start at the current position in the DS. The current
*position *		will be update to immediately following where the last
*character is
**		drawn. Embedded carriage contro (CR and LF) characters will be
*interpreted *		to move the current position rather than being rendered.
*This allows *		multi-line strings to be drawn.
*/
...
**
**	Description:
**		Render the specified character array using the currently
*selected font. *		It will start at the current position in the DS.
*The current position
**		will be update to immediately following where the last character
*is *		drawn. Embedded carriage contro (CR and LF) characters will be
*interpreted *		to move the current position rather than being rendered.
*This allows *		multi-line strings to be drawn.
*/
...
**		Render the specified character array using the currently
*selected font
**		starting at the specified position. The current position in the
*DS is
**		not modified. This function does not interpret embedded carriage
*control *		characters. If they are defined by the font, they will
*be rendered.
*/

bool MTDS::TextOut(HDS hds, int16_t xco, int16_t yco, int cchText,
                   char *rgchText) {
  PRM1A3B prm;
...
**
**	Description:
**		Return true if the first rectangle is equal to the second
*rectangle.
*/
...
bool MTDS::IsRectEqual(RCT *prct1, RCT *prct2) {
  ...
  if ((prct1->xcoLeft == prct2->xcoLeft) && (prct1->ycoTop == prct2->ycoTop) &&
      (prct1->xcoRight == prct2->xcoRight) &&
      (prct1->ycoBottom == prct2->ycoBottom)) {
    return true;
  } else {
    return false;
  }
...
void MTDS::SetRect(RCT *prct, int16_t xcoL, int16_t ycoT, int16_t xcoR,
                   int16_t ycoB) {
...
bool MTDS::IntersectRect(RCT *prct, RCT *prctOne, RCT *prctTwo) {
  ...
  */
  prct->xcoLeft = (prctOne->xcoLeft > prctTwo->xcoLeft) ? prctOne->xcoLeft
                                                        : prctTwo->xcoLeft;
  prct->xcoRight = (prctOne->xcoRight < prctTwo->xcoRight) ? prctOne->xcoRight
                                                           : prctTwo->xcoRight;
  ...
  */
  prct->ycoTop =
      (prctOne->ycoTop > prctTwo->ycoTop) ? prctOne->ycoTop : prctTwo->ycoTop;
  prct->ycoBottom = (prctOne->ycoBottom < prctTwo->ycoBottom)
                        ? prctOne->ycoBottom
                        : prctTwo->ycoBottom;
...
**
**	Description:
**		Draw a border around the specified rectangle using the current
*pen pattern,
...
bool MTDS::FrameRect(HDS hds, RCT *prct) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiFrameRect, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::FillRect(HDS hds, RCT *prct, HBR hbr) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiFillRect, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
...
bool MTDS::InvertRect(HDS hds, RCT *prct) {
  ...
  MtdsProcessCmdWr(clsCmdGdi, cmdGdiInvertRect, sizeof(prm), (uint8_t *)&prm, 0,
                   0);
