...
#define ropPatSrcNotOrDstOr                                                    \
  (31 + opnRopSrc + opnRopDst + opnRopPat) // (P | ~S) | D
...
** The upper 16 bits are for generic window styles. The lower 16 bits are for
*control
...
#define mrMtdsOrphan                                                           \
  3 // there is an orphaned memory block with no owning handle
...
#define mrMtdsFreeLink                                                         \
  11 // a free memory block has a backlink to a memory handle
