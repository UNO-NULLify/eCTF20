...
void MTDS_begin(PmodMTDS *InstancePtr, u32 GPIO_Address, u32 SPI_Address) {
  ...
  Xil_Out32(InstancePtr->GPIO_addr + 4,
            0b1111); // 0b1111 for input 0b0000 for output, 0b0001 for pin1 in
                     // pin 2 out etc.
...
**		Stops the device
*/
void MTDS_end(PmodMTDS *InstancePtr) { XSpi_Stop(&InstancePtr->MTDSSpi); }
...
int MTDS_SPIInit(XSpi *SpiInstancePtr) {
  ...
  Status =
      XSpi_CfgInitialize(SpiInstancePtr, &MTDSConfig, MTDSConfig.BaseAddress);
  ...
  Status = XSpi_SetOptions(
      ...
      (XSP_MASTER_OPTION | XSP_CLK_ACTIVE_LOW_OPTION | XSP_CLK_PHASE_1_OPTION) |
          XSP_MANUAL_SSELECT_OPTION); // Change these based on your SPI device
  if (Status != XST_SUCCESS) {
    return XST_FAILURE;
  }
...
**
**	Parameters:
**		PmodMTDS *InstancePtr	- the PmodMTDS object to communicate
*with
**
**	Return Value:
...
first byte into the specified register, then the next into the following
...
**	Parameters:
**		PmodMTDS *InstancePtr	- the PmodMTDS object to communicate
*with *		u8 reg			- the starting register to read from
...
*starting register, then the next from the following register. Data is stored
*into rData.
*/
...
**
**        Synopsis:
**				SetRegisterBits(&MTDSobj, MTDS_ADR_POWER_CTL,
*bPowerControlMask, fValue);
**        Parameters:
...
**
**        Description:
**				This function sets the value of some bits
*(corresponding to the bMask) of a register (indicated by bRegisterAddress) to 1
*or 0 (indicated by fValue).
**
**
*/
...
**
**        Return Values:
**                u8 					- a byte containing only
*the bits correspoding to the mask.
**
**        Errors:
**
**
**        Description:
**				Returns a byte containing only the bits from a
*register (indicated by bRegisterAddress), correspoding to the bMask mask.
**
**
*/
