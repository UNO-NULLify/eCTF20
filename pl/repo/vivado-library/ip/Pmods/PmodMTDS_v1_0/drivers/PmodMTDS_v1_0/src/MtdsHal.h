...
** The "standard" CS pin is taken from the symbol SS defined in the board
*variant
** header file for the selected board. This corresponds to the CS pin for the
*"default"
** SPI port, which is the one used by the standard SPI library.
** The "alternate" CS pin is supported on the Multi-Touch Display Shield when
*the CS
** select jumper is in the "alt" position. There is no alt CS pin on the
*PmodMTDS.
*/
...
** These pins are labeled HOST_INTA and HOST_INTB on the Multi-Touch Display
*Shield
** schematic, but are not actually used as interrupt pins. They are available as
*general
** purpose status pins between the host and the shield. Their use is not
*required by the
 ...
 */
// TODO: This value is not required for FPGA designs, but it should be fixed to
// be accurate
