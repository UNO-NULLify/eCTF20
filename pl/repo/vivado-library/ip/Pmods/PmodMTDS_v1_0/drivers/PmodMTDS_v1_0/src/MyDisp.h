 ...
 */
/*	Display Shield by Digilent, Inc. This library provides a simplified
 * access	*/
/*	model for use of the MTDS firmware. It uses and depends upon the main
 * MTDS	*/
/*	library for access to the shield. This library puts a simplified wrapper
 * on	*/
...
#if defined(__AVR__)
#define NUM_BUTTONS                                                            \
  12 // AVR devices don't have very much RAM, so not as many buttons
     // can be defined.
#else
...
public:
  ...
  bool setBacklight(int val) {
    return mtds.SetDisplayBacklight((uint32_t)val);
  };
  ...
  bool drawArc(int x1, int y1, int x2, int y2, int xr1, int yr1, int xr2,
               int yr2);
