/********************************************************************************/
/*																				*/
/*	MyDisp.cpp	--	Simple User Library for Multi-Touch Display
 * Shield			*/
/*																				*/
/********************************************************************************/
 ...
 */
/*	Display Shield. This library provides a simplified access model for use
 * of	*/
/*	the display shield firmware via the standard MTDS library. It uses the
 * MTDS	*/
...
**
**	Description:
**		Initialize the library for use. This initializes internal state
*of the *		interface object and initializes the underlying MTDS
*library for *		access to the display device.
*/
...
**
**	Description:
**		Releases all resources used by the library and terminates access
...
**
**	Description:
**		Set the specified color as the current foreground color. This is
*also used
**		to fill shapes such as rectangles, so also create a solid brush
*of the *		specified color and set it as the current brush.
*/
...
**
**	Parameters:
**		fTrans		- true for background transparent, false for
*background opaque
**
**	Return Values:
**		none
...
**	Parameters:
**		fTrans		- true for background transparent, false for
*background opaque *		clr			- transparency color to
*set.
**
**	Return Values:
**		none
...
**
**	Description:
**		Draw a line between the specified points using the current
*parameters *		set in the display DS.
*/
...
**
**	Description:
**		Draw the specified arc using the properties currently selected
*into the *		display DS.
*/
...
**
**	Description:
**		Draw and fill the specified rectangle. If fill is specified, it
*is filled *		with the solid brush corresponding to the currently
*selected foreground color. *		The perimeter is drawn using the current
*pen. If a NULL pen is selected, no *		perimeter is drawn.
*/
...
**
**	Description:
**		Draw the specified string of characters at the specified
*location. The string *		is drawn using the currently selected font,
*foreground and background colors.
**		The text origin corresponds to the upper left corner of the
...
**
**	Description:
**		This function will load a bitmap file from the SD card and draw
*it at the *		specified location. The bitmap is discarded after being
*drawn.
*/
...
**
**	Description:
**		This function will draw the specified bitmap at the specified
*location.
*/
...
bool MYDISP::drawImage(HBMP hbmp, int x, int y) {
  ...
  mtds.DrawBitmap(hdsDisp, (int16_t)x, (int16_t)y, bmpd.dxco, bmpd.dyco, hdsBlt,
                  0, 0);
...
**
**	Parameters:
**		name		- character string giving file name for image
*file
**
**	Return Values:
...
**
**	Description:
**		This function will read the specified image file and load it
*into *		memory as an internal, device-dependent bitmap.
*/
...
**		name2	- name of bitmap file for dispaly in touched state
*(optional) *		x, y	- origin of the button
**
**	Return Values:
**		none
**
...
**		fEn		- button show state, true=enabled,
*false=disabled
**
**	Return Values:
**		none
...
bool MYDISP::enableButton(int id, bool fEn) {
   ...
   */
  if (fEn) {
    /* Grab the background behind the button for use later in erasing the
     * button.
     */
...
**	Parameters:
**		id		- button identifier
**		st		- button state to draw: BUTTON_UP, BUTTON_DOWN
*or BUTTON_ERASE
**
**	Return Values:
**		none
    ...
    if ((rgbtn[id].fs & fsBtnImageUp) == 0) {
      ...
      */
      ...
      mtds.DrawBitmap(hdsDisp, rgbtn[id].rct.xcoLeft, rgbtn[id].rct.ycoTop,
                      rgbtn[id].dxco, rgbtn[id].dyco, hdsBlt, 0, 0);
      mtds.SetIntensity(hdsDisp, ityCur);
    }
...
**
**	Return Values:
**		Returns true if the specified button is visible and being
*touched
**
**	Errors:
**		none
...
**
**	Return Values:
**		Returns the ID of the last button "pressed" or -1 if no button
*has been pressed
**
**	Errors:
**		none
**
**	Description:
**		Scan the array of buttons to find the first one with the
*fsBtnActivated bit set *		in the flags. If one is found, clear the
*bit and return its ID.
*/
...
**
**	Description:
**		This function is the executive for maintaining the library state
*with *		regard to touch operations in the user interface. It polls the
*message *		queue for any touch messages, consumes them, maintaining
*the current *		state of the buttons and such based on the touch events
*seen.
*/
...
**
**	Description:
**		This function will check the button array and return the ID of
*the first
**		visible button that contains the specified coordinate. It
*returns -1 if *		no visible button contains the specified
*coordinate.
*/
