 ...
 */
/*		use functions in the new hardware abstraction layer to make
 * library		*/
...
**
**	Description:
**		Intialize the library, establish contact with the firmware
*running on *		the shield and initialize it for operation.
*/

bool MTDS::begin() { return begin(pinMtdsSelStd, frqMtdsSpiDefault); }
...
**
**	Description:
**		Tell the display shield that we aren't interested in talking to
*it anymore.
**		After calling this, it is necessary to call MTDS::begin() in
*order to start *		using it again.
*/

void MTDS::end() { fInitialized = false; }
...
**
**	Return Values:
**		Returns state of specified status pin. True if high, false if
*low.
**
**	Errors:
**		none
**
**	Description:
**		Check the state of the specified status pin and return true if
*high. The *		status pin being low indicates that the shield is
*signaling for *		attention.
*/

bool MTDS::GetStatusPin(int idPin) { return MtdsHalGetStatusPin(idPin); }
...
**		Depending on when this is called, the shield firmware may still
*be
**		in the process of power on initialization, it may be in the
*middle of *		sending back a status packet or sending a data packet.
*This is being
**		called because we have lost communication synchronization with
*the shield *		or we never had it to begin with. *		The
*procedure is as follows:
**			Send chnCmdSync bytes to the shield until it responds
*with a chnStaSync.
**			Because the first chnStaSync we see coming back could
*possible be data *			from a data packet being sent, we keep
*doing this until we have seen
**			ten chnStaSync bytes in a row.
**			Once we have seen ten chnStaSync bytes in a row, we
*assume that the shield *			communication state machine is
*in the sync state. We then send chnCmdStart
**			bytes to the shield until we get back a chnStaIdle.
**			It should not normally take more than two chnCmdStart
*bytes before we see
**			the first chnStaIdle, so if we send more than five
*without seeing the correct
**			response, we assume that the shield isn't working
*correctly and report an error. *		During this whole procedure, we
*check for timing out and if it takes too long, we *		assume that the
*shield isn't responding and report an error.
*/
...
bool MTDS::MtdsSyncChannel() {
  ...
  while (cntSync < cbSyncCount) {
    ...
    if (cntByte >= cbSyncLimit) {
      ...
      ** There are no communications delays when running under the simulator and
      *the
      ** return happens too fast there. Since communications errors aren't
      *really
      ** possible when running under the simulator this condition couldn't
      *actually
      ** come up anyway.
      */
      return false;
...
**
**	Description:
**		This function will send a command packet and optionally data-out
*packets *		to the shield.
**		It will then wait for the response packet to be received.
**		This function processes time-outs and will attempt to
*resynchronize *		communications with the shield if a time-out
*occur.
*/

void MTDS::MtdsProcessCmdWr(uint8_t cls, uint8_t cmd, uint16_t cbParam,
                            uint8_t *pbParam, uint16_t cbData,
                            uint8_t *pbData) {
  ...
  if (!MtdsWaitUntilShieldBusy()) {
    ...
    ** is not responding correctly and we either have a bad communications
    *channel,
  ...
  if (cbData != 0) {
    ...
    while (cbRem > 0) {
      ...
      ** before sending it. The shield status will go busy as soon as it starts
      *to
    ...
    if (!fStat) {
      ...
      ** or chnStaTimeout. chnStaReady and chnStaAbort have already been
      *handled,
  ...
  ** to read the status packet. The shield will start sending back chnStaDone
  *when
...
**
**	Description:
**		This function will send a command packet and then read data-in
*packets *		sent in response to the command. *		It will
*then wait for the response/status packet to be received. *		This
*function processes time-outs and will attempt to resynchronize *
*communications with the shield if a time-out occur.
*/

void MTDS::MtdsProcessCmdRd(uint8_t cls, uint8_t cmd, uint16_t cbParam,
                            uint8_t *pbParam, uint16_t cbData,
                            uint8_t *pbData) {
  uint16_t cbCur;
  uint8_t *pbCur;
