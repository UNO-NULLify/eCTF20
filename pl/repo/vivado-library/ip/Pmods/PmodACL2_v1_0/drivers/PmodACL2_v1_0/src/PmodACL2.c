...
XSpi_Config ACL2Config = {0, 0, 1, 0, 1, 8, 0, 0, 0, 0, 0};
...
**      Stops the device
*/
void ACL2_end(PmodACL2 *InstancePtr) { XSpi_Stop(&InstancePtr->ACL2Spi); }
...
XStatus ACL2_SPIInit(XSpi *SpiInstancePtr) {
  ...
  Status =
      XSpi_CfgInitialize(SpiInstancePtr, &ACL2Config, ACL2Config.BaseAddress);
