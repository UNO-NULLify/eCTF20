...
XSpi_Config XSpi_SF3Config = {0, 0, 1, 0, 1, 8, 2, 0, 0, 0, 0};
...
XStatus SF3_FlashWriteEnable(PmodSF3 *InstancePtr) {
  ...
  Status =
      XSpi_Transfer(&InstancePtr->SF3Spi, Buffer, NULL, SF3_WRITE_ENABLE_BYTES);
  if (Status != XST_SUCCESS) {
    return XST_FAILURE;
  }
 ...
 * @return   XST_SUCCESS if successful else XST_FAILURE.
 *
 * @note     The Buffer at BufferPtr is moved forward by the number of command
 *bytes used
 *
 ******************************************************************************/
...
XStatus SF3_BulkErase(PmodSF3 *InstancePtr) {
  ...
  TransferInProgress = TRUE;
  Status =
      XSpi_Transfer(&InstancePtr->SF3Spi, Buffer, NULL, SF3_BULK_ERASE_BYTES);
  if (Status != XST_SUCCESS) {
    return XST_FAILURE;
  }
...
XStatus SF3_SectorErase(PmodSF3 *InstancePtr, u32 Addr) {
  ...
  TransferInProgress = TRUE;
  Status =
      XSpi_Transfer(&InstancePtr->SF3Spi, Buffer, NULL, SF3_SECTOR_ERASE_BYTES);
  if (Status != XST_SUCCESS) {
    return XST_FAILURE;
  }
