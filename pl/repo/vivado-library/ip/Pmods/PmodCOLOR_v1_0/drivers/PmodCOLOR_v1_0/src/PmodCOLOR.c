...
XIic_Config COLOR_IicConfig = {0, 0, 0, 2};
...
void COLOR_WriteIIC(PmodCOLOR *InstancePtr, u8 reg, u8 *Data, int nData) {
  ...
  XIic_Send(InstancePtr->Iic.BaseAddress, InstancePtr->ChipAddr, out, nData + 1,
            XIIC_STOP);
 ...
 */
void COLOR_SetENABLE(PmodCOLOR *InstancePtr, u8 bits) {
  u8 bytes[2] = {COLOR_CMD_REPEAT_MASK | COLOR_RegENABLE, bits};
