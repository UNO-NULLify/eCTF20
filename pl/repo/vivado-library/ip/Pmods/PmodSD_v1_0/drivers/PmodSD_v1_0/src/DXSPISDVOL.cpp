...
/ 08/08/2016 Modified by Thomas Kappenman; Digilent Inc., for use with
Microblaze
/
/-------------------------------------------------------------------------*/
 ...
 */
/*-----------------------------------------------------------------------*/
void DXSPISDVOL::xmit_mmc(BYTE *buff, /* Data to be sent */
                          UINT bc     /* Number of bytes to send */
) {
  XSpi_Transfer(&SDSpi, buff, NULL, bc);
}

void DXSPISDVOL::rcvr_mmc(BYTE *buff, /* Pointer to read buffer */
...
int DXSPISDVOL::xmit_datablock(                  /* 1:OK, 0:Failed */
                               const BYTE *buff, /* 512 byte data block to be
                                                    transmitted */
...
BYTE DXSPISDVOL::send_cmd(BYTE cmd, /* Command byte */
                          DWORD arg /* Argument */
) {
  BYTE n, d, buf[6];
...
DSTATUS DXSPISDVOL::disk_status(void) { return Stat; }
...
DSTATUS DXSPISDVOL::disk_initialize(void) {
  ...
  for (n = 10; n; n--)
    xchg_spi(0xFF); /* Apply 80 dummy clocks and the card gets ready to receive
                       command */
...
DRESULT DXSPISDVOL::disk_read(
  ...
  cmd =
      count > 1 ? CMD18 : CMD17; /*  READ_MULTIPLE_BLOCK : READ_SINGLE_BLOCK */
...
#if _USE_IOCTL
DRESULT
DXSPISDVOL::disk_ioctl(uint8_t cmd, /* Control code */
