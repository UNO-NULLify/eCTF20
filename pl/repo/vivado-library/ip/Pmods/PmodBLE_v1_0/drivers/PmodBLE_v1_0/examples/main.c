...
// required definitions for sending & receiving data over the host board's UART
// port
...
void DemoInitialize() {
  EnableCaches();
  SysUartInit();
  BLE_Begin(&myDevice, XPAR_PMODBLE_0_S_AXI_GPIO_BASEADDR,
            XPAR_PMODBLE_0_S_AXI_UART_BASEADDR, BLE_UART_AXI_CLOCK_FREQ,
            115200);
}
...
void DemoRun() {
  ...
  xil_printf("Initialized PmodBLE Demo, received data will be echoed here, "
             "type to send data\r\n");
...
// initialize the system uart device, AXI uartlite for microblaze, uartps for
// Zynq
void SysUartInit() {
#ifdef __MICROBLAZE__
