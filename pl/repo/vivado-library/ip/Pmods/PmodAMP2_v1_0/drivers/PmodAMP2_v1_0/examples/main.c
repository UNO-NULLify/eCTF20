...
void DemoInitialize() {
  ...
  xil_printf("initializing\n\r");
  status =
      DemoIntcInitialize(&amp, &intc, INTC_DEVICE_ID, INTC_TMR_INTERRUPT_ID,
...
int DemoIntcInitialize(PmodAMP2 *InstancePtr, INTC *IntcInstancePtr,
  ...
  XIntc_Connect(IntcInstancePtr, InterruptId, (Xil_ExceptionHandler)Handler,
                InstancePtr);
  ...
  XTmrCtr_SetHandler(&InstancePtr->timer, (XTmrCtr_Handler)Handler,
                     &InstancePtr);
  ...
  status = XScuGic_CfgInitialize(IntcInstancePtr, IntcCfgPtr,
  ...
  status = XScuGic_Connect(IntcInstancePtr, InterruptId, Handler, InstancePtr);
