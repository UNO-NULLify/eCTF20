 ...
 *
 * @param   baseaddr_p is the base address of the PWM_ANALYZERinstance to be
 * worked on.
 *
 * @return
 *
...
XStatus PWM_ANALYZER_Reg_SelfTest(void *baseaddr_p) {
  ...
  for (write_loop_index = 0; write_loop_index < 4; write_loop_index++)
    PWM_ANALYZER_mWriteReg(baseaddr, write_loop_index * 4,
                           (write_loop_index + 1) * READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0; read_loop_index < 4; read_loop_index++)
    if (PWM_ANALYZER_mReadReg(baseaddr, read_loop_index * 4) !=
