 ...
 *
 * @param   baseaddr_p is the base address of the AXI_I2S_ADIinstance to be
 * worked on.
 *
 * @return
 *
...
XStatus AXI_I2S_ADI_Reg_SelfTest(void *baseaddr_p) {
  ...
  for (write_loop_index = 0; write_loop_index < 4; write_loop_index++)
    AXI_I2S_ADI_mWriteReg(baseaddr, write_loop_index * 4,
                          (write_loop_index + 1) * READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0; read_loop_index < 4; read_loop_index++)
    if (AXI_I2S_ADI_mReadReg(baseaddr, read_loop_index * 4) !=
