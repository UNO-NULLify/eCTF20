...
static int decrypt(const char *target_file, FILE *fp_s,
  ...
  } while ((endRead - CHUNK_SIZE) >
           ftell(fp_s)); // read until we are one buffer away
...
uint8_t decrypt30(FILE *encFile, struct metadata *meta, uint8_t *secret30) {
  uint8_t temphash[64] = {0};
  crypto_blake2b(temphash, (const uint8_t *)secret30,
                 strlen(secret30)); // turn long password into useable hash
...
uint8_t playShared(FILE *encFile, struct metadata *meta, uint8_t uid, char *pin) {
	...
	uint8_t nonce[24] = {0}; // generate nonce

	byte_me(mac, user_data[uid - 1].pvt_key + 64, 32); // generate mac


	byte_me(enc_pvt_key, user_data[uid - 1].pvt_key, 64); // generate enc_pvt_key
