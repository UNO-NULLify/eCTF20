...
int main(void) {
  ...
  puts("Region:");
  printf("Rand: %s\nPub: %s\nPriv: %s\n\n", region_data[0].rand,
         region_data[0].pub_key, region_data[0].priv_key);

  puts("Users:");
  printf("HW: %s\nPin: %s\nPub: %s\nPriv: %s\n", user_data[0].hw_secret,
         user_data[0].pin_hash, user_data[0].pub_key, user_data[0].pvt_key);

  puts("\ndecrypting user pin\n");

  if (crypto_pwhash_str_verify(user_data[0].pin_hash, "12345678",
                               strlen("12345678")) != 0) // strlen("12345678")
  {                                                      // strlen(PASSWORD)
    puts("nope");
  } else {
    puts("yup\n");
...
/*
    A function that checks if a string is within an array of region struct's
   name property param: char *region: the string which you are checking
...
/*
    A function that checks if a string is within an array of user struct's name
   property param: char *user: the string which you are checking
