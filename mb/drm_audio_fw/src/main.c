...
void load_song_md() {
  s.song_md.md_size = sizeof(c->song);
  s.song_md.owner_id = c->song.owner_id;
  memcpy(s.song_md.rids, (void *)c->song.region_ids, 64);
  memcpy(s.song_md.uids, (void *)c->song.sharedInfo, 64);
}
...
void query_player() {
  ...
  for (int i = 0; i < NUM_PROVISIONED_REGIONS; i++) {
    strcpy((char *)q_region_lookup(c->query, i),
...
/*void share_song() {
  ...
  if (shift) {
    memmove((void *)get_drm_song(c->song) + shift,
            (void *)get_drm_song(c->song), c->song.wav_size);
  }
...
void play_song() {
  ...
  // get WAV length
  length = (sizeof((void)c->song) - sizeof((void)c->song.buf));
  ...
  while (rem > 0) {
    ...
    Xil_MemCpy((void *)(XPAR_MB_DMA_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + offset),
               (void *)(get_drm_song(c->song) + length - rem), (u32)(cp_num));
